...
import com.vikings.handler.SectorHandler;
...
public class SectorRouter {
	...
	@Bean
  public RouterFunction<ServerResponse> sectorRoutes(SectorHandler sectorHandler) {
  	return 
	  	RouterFunctions
				.nest(path("/api/v1/sector"),
	  			route(GET("/get/{id}").and(accept(APPLICATION_JSON)), sectorHandler::get)
	  				...
	  				.andRoute(POST("/bulkoperation").and(accept(APPLICATION_JSON)).and(contentType(APPLICATION_JSON)), sectorHandler::bulkOperation)
